...
export default function SendBlock({ tn, documentId, refresh }) {
  ...
  const sendToEdds = async (data, { dryRun = false } = {}) => {
    const jwt = localStorage.getItem("jwt");
    const headers = jwt ? { Authorization: `Bearer ${jwt}` } : {};
    const qs = dryRun ? "?debug=1" : "";
    try {
      const res = await axios.post(`${URL}/services/edds/${qs}`, data, {
        headers,
        timeout: 30000,
      });
      return res?.data;
  ...
  const handleSend = async () => {
    try {
      ...
      if (toEdds) {
        const resp = await sendToEdds(payload, { dryRun: false });
        if (
          resp?.success === true ||
          resp?.ok === true ||
          resp?.raw ||
  ...
  const handleTest = async () => {
    try {
      ...
      setSending(true);
      const resp = await sendToEdds(payload, { dryRun: true });
      console.log("Тестовый JSON (ответ бэка):", resp);
      message.success(
        "Тестовый JSON отправлен на бэкенд (dryRun). Смотри логи."
      );
    } catch (e) {
      console.error("Ошибка тест-отправки:", e);
      message.error("Ошибка тест-отправки: " + (e?.message || "неизвестно"));
    } finally {
      setSending(false);
    }
  };
