...
export default function MapPanel({
  ...
  // Загрузка координат по FIAS кодам
  useEffect(() => {
    if (!fiasCodes.length || !url || !fiasCollection) {
      setResolvedPoints([]);
      return;
    }
    ...
    const fetchCoordinates = async () => {
      try {
        const uniqueCodes = [...new Set(fiasCodes.filter(Boolean))];

        // Формируем запрос в формате Strapi
        const params = new URLSearchParams();
        params.append("filters[fiasId][$in]", uniqueCodes.join(","));
        params.append("pagination[page]", "1");
        params.append("pagination[pageSize]", uniqueCodes.length.toString());

        const response = await fetch(`${url}/api/${fiasCollection}?${params}`, {
          headers: {
        ...
        const pointsWithCoords = (data.data || [])
          .map((item) => {
            const attrs = item.attributes || item;

            // Извлекаем координаты из разных возможных полей
            const lat = parseFloat(
              attrs.lat || attrs.latitude || attrs.geo_lat || attrs.geoLat
            );
            const lon = parseFloat(
              attrs.lon || attrs.longitude || attrs.geo_lon || attrs.geoLon
            );

            // Если координаты в массиве
            const coordsArray = attrs.coords || attrs.coordinates;
            let finalLat = lat;
            let finalLon = lon;

            if (Array.isArray(coordsArray) && coordsArray.length === 2) {
              finalLat = parseFloat(coordsArray[0]);
              finalLon = parseFloat(coordsArray[1]);
            }

            if (!isNaN(finalLat) && !isNaN(finalLon)) {
              return {
                id: attrs.fiasId || item.id,
                lat: finalLat,
                lon: finalLon,
              };
            }
        ...
        setResolvedPoints(pointsWithCoords);
      } catch (error) {
  ...
  // Подготовка features для карты
  const features = useMemo(() => {
    const sourcePoints = fiasCodes.length > 0 ? resolvedPoints : points;

    const featuresList = sourcePoints
      .map((point, index) => {
        // Извлекаем координаты из точки
        let lat, lon;
        ...
        } else if (Array.isArray(point.coordinates)) {
          [lat, lon] = point.coordinates.map(Number);
        } else if (Array.isArray(point.coords)) {
          [lat, lon] = point.coords.map(Number);
        }
        ...
        return {
          type: "Feature",
          id: point.id || point.fias || `point-${index}`,
          geometry: { type: "Point", coordinates: [lat, lon] },
          properties: {
            iconCaption: point.iconCaption || point.caption || "",
            hintContent: point.hintContent || point.caption || "",
    ...
    return { type: "FeatureCollection", features: featuresList };
  ...
  const omOptions = useMemo(
    () => ({
      clusterize: zoom < 16,
      gridSize: zoom > 14 ? 1 : zoom > 10 ? 32 : 64,
  ...
  return (
    <div style={{ position: "relative", width: "100%", height }}>
      ...
      >
        Точек на карте: {features.features.length}
      </div>
    </div>
  );
}
